...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXd> TrajectoryOptimizer::optimize_grid(
    ...
    Eigen::MatrixXd c3_grid = Eigen::MatrixXd::Constant(num_deps, num_arrs, nan);
    Eigen::MatrixXd v_inf_arr_grid = Eigen::MatrixXd::Constant(num_deps, num_arrs, nan);
    ...
    for (int i = 0; i < num_deps; ++i) {
        ...
        for (int j = 0; j < num_arrs; ++j) {
            ...
            try {
                ...
                double dv_arr = patched_conic.get_arrival_delta_v(v2, v_planet_arr, mu_arr, r_park_arr, e_arr);

                dep_dv_grid(i, j) = dv_dep;
                arr_dv_grid(i, j) = dv_arr;
                tot_dv_grid(i, j) = dv_dep + dv_arr;

                c3_grid(i, j) = (v1 - v_planet_dep).squaredNorm();
                v_inf_arr_grid(i, j) = (v2 - v_planet_arr).norm();
    ...
    return std::make_tuple(dep_dv_grid, arr_dv_grid, tot_dv_grid, c3_grid, v_inf_arr_grid);
}