...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXd> TrajectoryOptimizer::optimize_grid(
    ...
    for (int i = 0; i < num_deps; ++i) {
        // Extract 3D position (head) and 3D velocity (tail) for Earth
        Eigen::Vector3d r1 = dep_states.row(i).head<3>();
        Eigen::Vector3d v_earth = dep_states.row(i).tail<3>();
        ...
        for (int j = 0; j < num_arrs; ++j) {
            ...
            // Extract 3D position (head) and 3D velocity (tail) for Mars
            Eigen::Vector3d r2 = arr_states.row(j).head<3>();
            Eigen::Vector3d v_mars = arr_states.row(j).tail<3>();

            try {
                auto [v1, v2] = solver.solve(r1, r2, tof, long_way);

                double dv_dep = patched_conic.get_departure_delta_v(v1, v_earth);
                double dv_arr = patched_conic.get_arrival_delta_v(v2, v_mars);
